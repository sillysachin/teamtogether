...
$isValidPost=$dbobj->validatePost(array("cid","tid","mid"));

if ($isValidPost) {
    $connection=$dbobj->getDB();
    $epost=$dbobj->escapePost(array("cid","tid","mid"),$connection);
    $get_query="SELECT id,time,modified_time,subject from rnbje_kunena_messages WHERE catid='{$epost["cid"]}' AND thread='{$epost["tid"]}' AND id>'{$epost["mid"]}' ORDER BY id ASC";
    ...
    for($i=0;$i<$len;$i++)
    {
        ...
        echo "{ \"id\":\"{$ids[$i]}\",";
        echo "\"message\":",json_encode(mysqli_fetch_assoc($get_res)['message']),",";
        if($mtime[$i]==null)
            echo "\"time\":\"{$time[$i]}\",";
        else
            echo "\"mtime\":\"{$mtime[$i]}\",";
