...
class KunenaControllerTopic extends KunenaController {
	...
	public function mobNotify($subject,$category,$isedited=0,$dept,$isDeleted,$isEdited,$message_id)
	{
if($message_id==0)
$message_id=file_get_contents('http://ts.icias2015.org/university/app_files/returnMax.php?catid={$category}');
		$url = 'http://ts.icias2015.org/university/tspear/cigniter/index.php';
		$fields = array();
	$fields['message']="";
	$fields['message'].=($isedited==1)?"Edit: ":"";
	$fields['message'].="[".$category."]"." {$subject}";
	$fields['username']="talentspear";
$fields['mid']=$message_id;
$fields['deleted']=$isDeleted;
$fields['edited']=$isEdited;
	$fields['password']="talentspear_university";
...
return intval($result);		
	...
	public function post() {
		...
		if (!$this->id) {
			...
			switch($cato)
			{
			  case "Civil":$dept="CV";break;
			  case "Placement Forum":$dept="ALL";break;
			  case "SWO Circulars":$dept="ALL";break;
			  case "Mechanical":$dept="ME";break;
			  case "EEE":$dept="EE";break;
			  case "Automobile":$dept="AU";break;
			  case "Departments":$dept="ALL";break;
			  default:$dept=$this->getCategory()->name;break;
...
$cato=$this->getCategory()->name;
	...
	public function edit() {
		...
		$total1=$this->mobNotify($fields['subject'],$this->getCategory()->name,1,$dept,0,1,$this->mesid);
		$this->app->enqueueMessage ( JText::_ ( 'COM_KUNENA_POST_SUCCESS_EDIT' )." Notification Sent to ".$total1." students." );
		if ($message->hold == 1) {
...
$this->mobNotify($fields['subject'],$this->getCategory()->name,1,$dept,1,0,$this->mesid);
		} else {
			// Delete topic
