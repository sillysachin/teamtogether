  ...
  Ember.ResourceAdapter = Ember.Mixin.create({
    ...
    */
    _resourceRequest: function(params) {
      params.url = this._resourceUrl(params);
...
Ember.Resource = Ember.Object.extend(Ember.ResourceAdapter, Ember.Copyable, {
  ...
  */
  _resourceUrl: function(params) {
    var url = this.resourceUrl,
        id = this._resourceId();

    if (params) {
      if (params.type === 'DELETE') {
        url += '&task=' + this.resourceName + '.' + params.type.toLowerCase();
      }
    }
...
Ember.ResourceController = Ember.ArrayController.extend(Ember.ResourceAdapter, {
  ...
  */
  ...
  /**
    Replace this controller's contents with an request to `url`
  */
  findAllWithoutClearLoadOnlyNew: function(callback) {
    var self = this;

    return this._resourceRequest({type: 'GET'})
      .done(function(json) {
        self.loadAllNew(json);
        callback();
      });
  },
  /**
      ...
      if (rt.prototype.resourceUrl === undefined) {
        for (var i = rt.PrototypeMixin.mixins.length - 1; i >= 0; i--) {
          ...
          if (m.properties !== undefined && m.properties.resourceUrl !== undefined) {
            return m.properties.resourceUrl + '&view=' + m.properties.resourceNamePlural + '&format=json';
          }
        }
      }
      else {
        return rt.prototype.resourceUrl + '&view=' + rt.prototype.resourceNamePlural + '&format=json';
      }
    }

    return this.resourceUrl + '&view=' + rt.prototype.resourceNamePlural + 's&format=json';
  }
});
